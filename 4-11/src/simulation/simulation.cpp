...
#include "PositionCalculation/PositionCalculation.hpp"
#include "MiscTools/MiscTools.hpp"
...
void Simulation::initialize_positions(){
  ...
  //EarthPos.push_back(-2.741147560901964e10);
  //EarthPos.push_back(1.452697499646169e11);
  //EarthPos.push_back(1.907499306293577e7);
  
  
  MoonPos.push_back(-2.621966950910588e10);
  MoonPos.push_back(1.444221966223293e11);
  MoonPos.push_back(-1.481725083564967e7);
  //MoonPos.push_back(-2.750334415265606e10);
  //MoonPos.push_back(1.449229071220244e11);
  //MoonPos.push_back(1.107672420730442e7);
}
...
void Simulation::update_moons_position_sv(double dt){
  ...
  double oldxAcceleration = sv.computeXAccel(x1,y1,z1,x2,y2,z2,earth_mass);
...
double Simulation::determine_if_solar_eclipse(std::string start_date, double time){

  MiscTools tool;
  PositionCalculation pc;
  ...
  if((abs(proj_b_a_perp.norm())<((6.371e6))) and(earth.norm()>moon.norm())) {

    std::vector<double> results;

    results = pc.Calculate_lat_and_long(EarthPos, MoonPos);

    std::cout << "LAT: " << results.at(0) << ". LONG: " << results.at(1) << ".\n";
